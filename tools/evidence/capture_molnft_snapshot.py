...
def capture(args: argparse.Namespace) -> tuple[pathlib.Path, dict[str, Any]]:
    ...
    for idcode in SAMPLE_IDCODES:
        ...
        # Contract strings are line-wrapped. Remove ASCII whitespace, restore standard
        # padding when needed, and then validate the normalized base64 strictly. The
        # unmodified JSON-RPC response remains preserved in raw/.
        normalized_b64 = "".join(combined_b64.split())
        normalized_b64 += "=" * ((-len(normalized_b64)) % 4)
        compressed = base64.b64decode(normalized_b64, validate=True)
